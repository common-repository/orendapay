...
Implemente transações por cartão de crédito ou débito transparente, pix e boletos bancários no Checkout do seu Woocommerce usando a solução de pagamento Orenda Pay.
...
= 1.0 =
Essa é a primeira versão de lançamento, sem relatos de bugs ou problemas.

= 2.0 =
Implementação do cartão de crédito, com checkout transparente

= 2.5 =
Configuração para definir qual será o status do pedido quando o pagamento for confirmado automaticamente.

= 3.0 =
Implementação do cartão de débito, com checkout transparente.

== Changelog == 

= 1.0 =
Primeira versão compatível com WordPress Woocommerce
...
= 3.5 =
Validação no checkout para exigir a seleção do método de pagamento OrendaPay, se tiver mais de uma opção.

= 4.0 =
Foi implementado nesta versão o boleto parcelado. Você configura na administração e no checkout o usuário terá essa opção de pagamento.

= 4.1 =
Implementação do split de pagamento, divida seus recebimentos em percentual entre outras contas OrendaPay ou e-mails não cadastrados OrendaPay.

= 4.2 =
Implementação do pagamento por PIX.  

= 4.3 =
Mensagem de aprovação na hora no Checkout se pagamento via Pix.
